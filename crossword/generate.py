...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for word1 in self.domains[x].copy():
            if any(word2[j] == word1[i] for word2 in self.domains[y]):
                continue
    ...
    def ac3(self, arcs=None):
        ...
        """
        if not arcs:
            arcs = [arc for arc in self.crossword.overlaps if self.crossword.overlaps[arc]]
        while arcs:
            ...
            if self.revise(var1, var2):
                if not self.domains[var1]:
                    return False
                arcs.extend((
                    (z, var1) for z in self.crossword.neighbors(var1)
                    if (z, var1) not in arcs
                ))
        return True
    ...
    def consistent(self, assignment):
        ...
        """
        # check if all the words are distinct
        if len(assignment.values()) != len(set(assignment.values())):
            return False
        # check if arc consistent
        for (var1, var2), overlap in self.crossword.overlaps.items():
            if overlap and var1 in assignment and var2 in assignment:
                i, j = overlap
        ...
        for word1 in self.domains[var]:
            ...
            for neighbor in neighbors:
                ...
                for word2 in self.domains[neighbor]:
                    if word1[i] != word2[j]:
                        counter += 1
            ordered.append((counter, word1))
        return [word for n, word in sorted(ordered, key=lambda value: value[0])]
    ...
    def backtrack(self, assignment):
        ...
        for word in self.order_domain_values(var, assignment):
            ...
            if self.consistent(assignment):
                domains = {v: self.domains[v].copy() for v in self.domains}
                arcs = [(z, var) for z in self.crossword.neighbors(var) if z not in assignment]
                if arcs and self.ac3(arcs):
